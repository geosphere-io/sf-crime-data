    ...
    var MAPBOX_MAP_STYLE_ID = 'mapbox.streets';//'lightfox.1n10e3dp';
...
/****/
    ...
    function _drawIncidents(incidentGeoJson)
	{
		...
		incidentLayer.setGeoJSON(ResultJSON).eachLayer(function(layer)
		{
          ...
          //this line below changes icon using leaflet Icon objects.
			switch(true)
			{
				case (layer.feature.properties.incident_category==="Assault")://""AGGRAVATED ASSAULT"):
				...
				case(layer.feature.properties.incident_category==="Arson")://"ARSON"):
				...
				case (layer.feature.properties.incident_category==="Burglary")://"BURGLARY"):
				...
				case (layer.feature.properties.incident_category==="DATING VIOLENCE"):
				...
				case(layer.feature.properties.incident_category==="DOMESTIC VIOLENCE"):
				...
				case(layer.feature.properties.incident_category==="Drug Violation")://"DRUG-RELATED VIOLATIONS"):
				...
				case(layer.feature.properties.incident_category==="HATE CRIMES"):
				...
				case(layer.feature.properties.incident_category==="Liquor Laws")://"LIQUOR LAW VIOLATIONS"):
				...
				case(layer.feature.properties.incident_category==="Motor Vehicle Theft")://"MOTOR VEHICLE THEFT"):
				...
				case(layer.feature.properties.incident_category==="Robbery")://"ROBBERY"):
				...
				case(layer.feature.properties.incident_category==="Sex Offense")://"SEX OFFENSES"):
				...
				case(layer.feature.properties.incident_category==="STALKING"):
				...
				case(layer.feature.properties.incident_category==="Weapons Offence")://"WEAPONS POSSESSION"):
			...
			{
				case ("Cite or Arrest Adult").toLowerCase():
					if ($("#Cite_or_Arrest_Adult").is(':checked'))
						show = show && true;
					else
						show = show && false;
				break;

				case ("Cite or Arrest Juvenile").toLowerCase():
					if ($("#Cite_or_Arrest_Juvenile").is(':checked'))
						show = show && true;
					else
						show = show && false;
				break;

				case ("Exceptional Adult").toLowerCase():
					if ($("#Exceptional_Adult").is(':checked'))
						show = show && true;
					else
						show = show && false;
				break;

				case ("Exceptional Juvenile").toLowerCase():
					if ($("#Exceptional_Juvenile").is(':checked'))
						show = show && true;
					else
						show = show && false;
				break;

				case ("Open or Active").toLowerCase():
					if ($("#Open_or_Active").is(':checked'))
						show = show && true;
					else
						show = show && false;
				break;
    ...
    function _buildIncidentPopupContent(properties) {
        var newDate = properties.incident_date;
        var formattedDate = newDate.slice(5,7) + "/" + newDate.slice(8,10) + "/" + newDate.slice(0,4);
        return properties.incident_description + " on " + formattedDate;
    }
	...
	function _toCSV() {
		...
		LineArray[0] = '"incident_date","incident_datetime","analysis_neighborhood","incident_category","incident_category","resolution","police_district","incident_description","incident_number"\r\n';

		LineForShow.forEach
		(
			function (item)
			{
				LineArray[LineArray.length] = 	'"' + item.properties.incident_date + '","' +
													item.properties.incident_datetime + '","' +
													item.properties.analysis_neighborhood + '","' +
													item.properties.incident_category + '","' +
													item.properties.incident_category + '","' +
													item.properties.resolution + '","' +
													item.properties.police_district + '","' +
													item.properties.incident_description + '","' +
													item.properties.incident_number + '"\r\n';
			}
		);
	...
	function _GetOnlyForSave()
	{
		...
		(
			...
			{
				var show = false;
				switch(item.properties.incident_category)
				{
					case "Assault"://"AGGRAVATED ASSAULT":
						if ($("#assault").is(':checked'))
							show = true;
					break;
					...
					case "Drug Violation"://"DRUG-RELATED VIOLATIONS":
						if ($("#drug-possession").is(':checked'))
							show = true;
					break;
					...
					case "Liquor Laws"://"LIQUOR LAW VIOLATIONS":
						if ($("#liquor-violation").is(':checked'))
							show = true;
					break;

					case "Motor Vehicle Theft"://"MOTOR VEHICLE THEFT":
						if ($("#vehicle-theft").is(':checked'))
							show = true;
					break;
					...
					case "Weapons Offence"://"WEAPONS POSSESSION":
						if ($("#weapons-possession").is(':checked'))
							show = true;
					break;
...
/*************************************************OLD***********************************************/
	...
	function _ShowDataSet() {
		...
		LineForShow.forEach (
			function (item) {
				oldDate = item.properties.incident_date;
				newDate = oldDate.slice(5,7) + "-" + oldDate.slice(8,10) + "-" + oldDate.slice(0,4);

				table += 	'\n\n\n\n<tr>' +
										'\n\n\n\n\n<td class="date"><span>' + newDate + '</td>' +
										'\n\n\n\n\n<td class="time"><span>' + item.properties.incident_datetime + '</span></td>' +
										'\n\n\n\n\n<td class="address"><span>' + item.properties.analysis_neighborhood + '</span></td>' +
										'\n\n\n\n\n<td class="category-sfpd"><span>' + item.properties.incident_category + '</span></td>' +
										'\n\n\n\n\n<td class="category-cs"><span>' + item.properties.incident_category + '</span></td>' +
										'\n\n\n\n\n<td class="resolution"><span>' + item.properties.resolution + '</span></td>' +
										'\n\n\n\n\n<td class="district-sfpd"><span>' + item.properties.police_district + '</span></td>' +
										'\n\n\n\n\n<td class="description"><span>' + item.properties.incident_description + '</span></td>' +
										'\n\n\n\n\n<td class="incident-number"><span>' + item.properties.incident_number + '</span></td>' +
									'\n\n\n\n</tr>' +
									'\n\n';
				}
			);
